...
function CognizantITS() {
}

CognizantITS.deHighlightPreviousElement = function () {
    Util.deghlightElement(previousElement);
};

CognizantITS.sendSpiedObject = function (element, flag) {
    var data = {};
    data.type = "Object";
...
CognizantITS.sendHealResult = function (data) {
    data.action = "heal";
    sendValues(JSON.stringify(data));
};

CognizantITS.sendRecordedObject = function (element, method, input) {
    var data = {};
    data.action = "record";
    data.method = method;
    data.input = input;
...
CognizantITS.getFrameValue = function () {
    return getFrameData();
};
...
function sendValues(values) {
    browser.runtime.sendMessage({ type: "sendMessageToServer", data: values });
}
